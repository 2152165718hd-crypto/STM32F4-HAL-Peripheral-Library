...
{
#if USE_HARDWARE_SPI
    /* * 修正：HAL_SPI_Transmit 的 Size 参数是 uint16_t，最大只能发送 65535 字节。
     * 当发送整屏图片(如240*320*2 = 153600字节)时会溢出导致显示不全。
     * 解决方法：将大数据包拆分为多个小于 65535 的小包发送。
     */
    uint16_t chunk_size;
    
    while (len > 0)
    {
        /* 每次最多发送 65532 字节 (保持偶数，虽然8位模式无所谓，但习惯上留点余量) */
        if (len > 65532)
        {
            chunk_size = 65532;
        }
        ...
        {
            chunk_size = (uint16_t)len;
        }

        /* 硬件SPI发送 */
        HAL_SPI_Transmit(&hspi1, (uint8_t *)data, chunk_size, 1000);
 ...
 *
 * 修复说明：
 *   - 重要：st7789_set_window后CS被拉高，需要再次拉低才能发送数据
 *   - 这是之前代码的bug修复，避免缓冲区数据被忽略
 */
